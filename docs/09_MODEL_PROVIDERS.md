...
- **A model is Provider → Model → Capability, and holds a SET of capabilities** (Milestone 15). The capabilities are `article_generation`, `seo_review`, `research`, `quality_review`, `title_generation` and `keyword_expansion`; the first two are routed to today, and the rest are assignable and labelled as not yet routed. One model may hold several — the best article writer is not automatically the best SEO reviewer, but one model can legitimately be both. Capabilities live in `ai_model_capabilities`; `ai_provider_models.purpose` is deprecated-but-preserved and no longer read.
- **Which model runs a task is configuration, not code.** The Model Router resolves a task type to the model set as default *for that capability* — defaults are per capability, so "default article model" and "default SEO model" are independent choices. Nothing hard-codes a vendor, and the frontend cannot select one. A model not registered for a task is refused rather than used, so purpose separation is enforced by the router and not merely by the picker.
- **A model is validated before it is saved** (Milestone 15). `modelValidation.service.ts` runs `configuration → endpoint → credential → catalogue → live probe → normalisation` and reports every stage, so a failure names the actual problem — a Base URL pointing at a provider's website rather than its API root is reported as exactly that, with the correction quoted. Stages 1–4 cost nothing.
- **Test Model never writes an article.** Four-word prompt, output capped at 16 tokens, response discarded. A free model is tested outright; a paid or unpriced one requires explicit confirmation and is quoted an estimated cost first. Unknown pricing is treated as paid, never as free.
